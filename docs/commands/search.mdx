...
The `kagi search` command performs a search query against Kagi's search index and returns results in a structured format. By default, output is JSON suitable for programmatic processing. With the `--format pretty` flag, results are formatted for human readability.
...
**Note:** `--format pretty` only changes output formatting. It does not affect:
...
kagi search --lens 2 --format pretty "rust async"
```
...
2. **Pretty for humans:** Use `--format pretty` for interactive use
