---
title: "search"
description: "Complete reference for *kagi* search command - syntax, filters, authentication, output formats, and examples."
---
...
This matters because the Kagi Search API is still base-search only in this CLI. As soon as you add live search filters like `--region`, `--time`, `--order`, `--verbatim`, or `--personalized`, the request becomes session-only.
...
- `kagi batch` - run multiple searches with the same search filters
- `kagi assistant` - continue research with Kagi Assistant
- `kagi summarize` - summarize a page after you find it
