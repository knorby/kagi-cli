...
**Scenario 1: API-first mode works**
```bash
...
**Scenario 2: API-first mode rejected, fallback to session**
```bash
export KAGI_API_TOKEN='valid_token'
...
**Scenario 3: Default session-first behavior**
```bash
...
3. **Configured search preference:** Base search defaults to session unless `[auth.preferred_auth] = "api"`
